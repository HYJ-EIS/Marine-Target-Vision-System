...
def _apply_formal_v3_config(config: Any) -> dict[str, tuple[bool, Any]]:
    old_values = {
        key: (hasattr(config, key), getattr(config, key, None))
        for key in FORMAL_V3_CONFIG_OVERRIDES
    }
    for key, value in FORMAL_V3_CONFIG_OVERRIDES.items():
        setattr(config, key, value)
    return old_values


def _restore_config_values(config: Any, old_values: dict[str, tuple[bool, Any]]) -> None:
    for key, (had_attr, value) in old_values.items():
        if had_attr:
            setattr(config, key, value)
...
def run_speed_benchmark(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    old_config = _apply_formal_v3_config(Config)
    try:
        ...
        _restore_config_values(Config, old_config)
