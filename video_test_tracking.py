...
def main() -> None:
    ...
    while True:
        ...
        # 统计 ID 切换：当前帧新出现的 ID 中有多少与上一帧完全不重叠的
        current_ids = {b["track_id"] for b in tracked_boxes}
        disappeared = prev_track_ids - current_ids
        appeared = current_ids - prev_track_ids
        # 简单估计：上一帧消失且同时有新 ID 出现，判定为可能的 ID 切换
        ...
        if processed % 50 == 0:
            ...
            print(
                ...
                f"检测: {len(boxes)} | 活跃轨迹: {len(current_ids)} | "
                f"ID切换估计: {id_switch_count} | 实际处理fps: {fps_real:.1f}"
            )
    ...
    print(f"[完成] ID切换估计: {id_switch_count}")
