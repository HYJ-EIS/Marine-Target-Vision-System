...
1. 针对小目标 MOT，提出了一种感知检测不稳定性的轨迹生命周期建模方法，将高置信检测、低置信检测和连续漏检共同纳入候选确认、活跃保留、丢失恢复和移除决策。
...
\mathcal{T}_i^t=
\left(
z_i^t,\ b_i^t,\ e_i^t,\ h_i^t,\ m_i^t,\ r_i^t,\ \mathcal{S}_i^t,\ \mathcal{L}_i^t
\right),
$$

其中 \(z_i^t\) 是生命周期状态，\(b_i^t\) 是轨迹框，\(e_i^t\) 是证据得分，\(h_i^t\) 是命中次数，\(m_i^t\) 是漏检次数，\(r_i^t\) 是真实检测命中次数，\(\mathcal{S}_i^t\) 是观测来源历史，\(\mathcal{L}_i^t\) 是近期低置信检测历史。生命周期状态集合为：
...
**输入输出。** 输入为第 \(t\) 帧检测集合 \(D_t\)，每个检测 \(d\) 包含边界框 \(b(d)\)、置信度 \(s(d)\) 和类别标签。输出为带来源标签的 observation groups，来源集合为 \(\{\text{high\_det}, \text{low\_det}\}\)。其中，高置信观测主要用于普通候选生成和可靠状态更新，低置信观测用于弱证据累积、低置信候选生成和丢失轨迹重关联。
...
$$
G_t=\{g_j^t\},\qquad
Q(g_j^t)\subseteq \{\text{high\_det},\text{low\_det}\}.
$$
...
**机制描述。** 对于被匹配的轨迹，ELTTrack 根据 observation group 中不同来源的置信度计算正证据增量：
