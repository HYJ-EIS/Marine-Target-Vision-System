...
### MS-DC lost reacquire variants

`reacquire_every_frame_low_score` 保持 formal `msdc_v3` 的其它配置不变，仅将 LOST 轨迹重捕获从 `MSDC_REACQUIRE_INTERVAL=5` 改为每帧尝试（`MSDC_REACQUIRE_INTERVAL=1`），并将恢复分数阈值从 `MSDC_REACQUIRE_SCORE=1.5` 降到 `1.0`。该变体用于和 `msdc_v3` 比较关键断点能否接回；速度脚本可通过 `tools/evaluation/msdc_speed_benchmark.py --msdc-variant reacquire_every_frame_low_score` 记录对应的无渲染分阶段耗时。
