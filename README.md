...
相关环境变量：`MSDC_REMOVED_GUARD_IOU_THRESH`、`MSDC_REMOVED_GUARD_CENTER_DIST`、`MSDC_REMOVED_RECOVERY_ENABLE`、`MSDC_REMOVED_RECOVERY_REQUIRE_CLASS_MATCH`、`MSDC_REMOVED_RECOVERY_MAX_AGE`、`MSDC_REMOVED_RECOVERY_MIN_IOU`、`MSDC_REMOVED_RECOVERY_MAX_CENTER_DIST`、`MSDC_REMOVED_RECOVERY_MIN_SCORE`。正式 removed-recovery 对比应同时包含 `msdc_v3` 和 `removed_recovery_off`，并交付 MOT summary、可视化 MP4、diagnostics、速度统计和 stage timing CSV。

### MS-DC low-det position update

`MSDC_LOW_UPDATE_ACTIVE_BOX_ENABLE` 控制 pure `low_det` 匹配到 ACTIVE 轨迹时是否刷新 primary box，默认 `0`，用于保持历史 `msdc_v3` 行为；`MSDC_LOW_UPDATE_LOST_BOX_ENABLE` 控制 pure `low_det` 通过 reacquire 匹配到 LOST 轨迹时是否刷新 primary box，默认 `1`，因为历史代码已经对 LOST reacquire 刷新位置。消融变体 `low_position_update_all_off` 将两者同时关闭，`low_position_update_on` 将两者同时打开；与 removed-ID recovery 交叉时使用 `removed_recovery_off_low_position_update_all_off` 和 `removed_recovery_off_low_position_update_on`。速度脚本支持 `tools/evaluation/msdc_speed_benchmark.py --msdc-variant <variant>`，用于记录对应变体的无渲染分阶段耗时。
...
- MS-DC 正式和 replay benchmark 会在 per-GT diagnostics 可用时额外写出 `diagnostics/msdc_diagnostic_summary.csv`，汇总低分候选创建/确认/召回、继承机会/成功率、重捕获尝试/机会/成功率、removed-ID recovery 尝试/成功/回退新 ID、碎片化和轨迹断裂诊断。
