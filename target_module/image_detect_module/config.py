...
class Config:
    ...
    # active/lost 生命周期超时和每帧重捕参数
    MSDC_ACTIVE_MISSING_PATIENCE = 2
    MSDC_LOST_MAX_AGE = _env_int("MSDC_LOST_MAX_AGE", 40)
    MSDC_REACQUIRE_INTERVAL = _env_int("MSDC_REACQUIRE_INTERVAL", 1)
    MSDC_REACQUIRE_SCORE = _env_float("MSDC_REACQUIRE_SCORE", 0.8)
    ...
    MSDC_PENDING_RECOVERY_FRAMES = _env_int("MSDC_PENDING_RECOVERY_FRAMES", 1)
