...
对所有红外视频分别用两个模型跑追踪，解析统计指标并输出对比表格。
...
_PATTERNS = {
    ...
    "id_switches": re.compile(r"\[完成\]\s*ID切换估计:\s*(\d+)"),
}
...
def main() -> None:
    ...
    header = f"{'视频':<10} {'模型':<8} {'处理帧数':>8} {'总检测数':>8} {'检测/帧':>8} {'ID切换':>6} {'速度(fps)':>10}"
    print(header)
    print("-" * len(header))
