...
def main():
    ...
    for video_path in VIDEOS:
        ...
        run_cmd(
            ...
            [PYTHON, os.path.join(ROOT, "video_test_tracking.py"),
             "--input", video_path,
             "--output", out_full,
             "--tracker", "botsort",
             "--max-frames", mf],
        ...
        # ── 3. FPS 梯度统计 ────────────────────────────────────────
        out_csv = os.path.join(ROOT, "results", f"stats_{tag}.csv")
        run_cmd(
            f"[{tag}] FPS 梯度统计",
            [PYTHON, os.path.join(ROOT, "tracking_stats.py"),
             "--input", video_path,
             "--tracker", "botsort",
    ...
    for tag in results_summary:
        ...
        print(f"  - results/stats_{tag}.csv           (FPS 统计 CSV)")
        print(f"  - results/stats_{tag}.png           (FPS 统计图表)")
